...
- **Special case**: When all valid lines in a category are excluded (`exclude=True`), the category percentage is set to 100% (indicating the mapping task is complete)
...
- **Excluded line handling**: Lines marked as `exclude=True` are always excluded from calculations regardless of their `field_name` status, and are counted as regular excluded fields
